            ...
            for (const r of reposData) {
                ...
                try {
                    for (let page = 1; page <= 4; page++) {
                        let commitsRes = await axios_1.default.get(`https://api.github.com/repos/${username}/${r.name}/commits?author=${username}&per_page=100&page=${page}`, { headers, timeout: 3000 });
                        if (!Array.isArray(commitsRes.data) || commitsRes.data.length === 0) {
                            if (page === 1) {
                                commitsRes = await axios_1.default.get(`https://api.github.com/repos/${username}/${r.name}/commits?per_page=100&page=1`, { headers, timeout: 3000 });
                            }
                            else {
                                break;
                            }
                    ...
                    try {
                        const commitsRes = await axios_1.default.get(`https://api.github.com/repos/${username}/${r.name}/commits?per_page=100`, { headers, timeout: 3000 });
