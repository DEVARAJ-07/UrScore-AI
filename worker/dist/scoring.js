...
exports.computeUrScore = computeUrScore;
function computeUrScore(resume, github, leetcode, portfolio) {
    ...
    const githubDeps = new Set();
    github.repos.forEach(repo => {
        if (repo.language)
            githubLanguages.add(repo.language.toLowerCase());
    ...
    let overall_score = (skill_verification * 0.25) +
        (commit_quality * 0.20) +
